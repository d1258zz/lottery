...
namespace D1258zz\Lottery\Commands;
...
use D1258zz\Lottery\Events\LotteryEnterChanged;
use D1258zz\Lottery\Events\LotteryWasEntered;
use D1258zz\Lottery\Lottery;
use D1258zz\Lottery\LotteryRepository;
...
class EnterLotteryHandler
{
    ...
    public function handle(EnterLottery $command)
    {
        ...
        if ($existingParticipant) {
            throw new ValidationException([
                'lottery' => $this->translator->trans('d1258zz-lottery.forum.composer_discussion.in_queue_alert'),
            ]);
        }

        if($lottery->getAttribute('enter_count')>$lottery->getAttribute('max_participants')){
            throw new ValidationException([
                'lottery' => $this->translator->trans('d1258zz-lottery.forum.too_many_participants'),
            ]);
        }
        ...
        if (!$this->userMeetsConditions($actor, $lottery)) {
            throw new ValidationException([
                'lottery' => $this->translator->trans('d1258zz-lottery.forum.composer_discussion.no_permission_alert'),
            ]);
        }

        if($lottery->getAttribute('price') > $actor->getAttribute('money')){
            throw new ValidationException([
                'lottery' => $this->translator->trans("d1258zz-lottery.forum.modal.not_enough").' '.$this->translator->trans("d1258zz-lottery.forum.modal.money"),
            ]);
        }
    ...
    protected function checkCondition(User $user, $condition)
    {
        ...
        if (!$this->meetsCondition($value, $operator, $threshold)) {
            $errorMessageKey = "d1258zz-lottery.forum.modal.{$condition['operator_type']}";
            throw new ValidationException([
                'lottery' => $this->translator->trans("d1258zz-lottery.forum.modal.not_enough").' '.$this->translator->trans($errorMessageKey),
            ]);
        }
    }
